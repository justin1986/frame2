<?php require "../../frame.php";
	...
	if($_REQUEST['id']){
		...
		$item->view_id = $_REQUEST['view_id'];	
	}
	
	function assing_to_js($item,$js_name){
		?>
		<script>			
		var <?php echo $js_name;?> = new view_module_class();
		...
		<?php echo $js_name;?>.view_id = '<?php echo $item->view_id;?>';
		</script>
		<?php
	}
	...
	$category = $category->find('all',array('conditions' => '1=1'));
...
<div id="view_item_container">
	...
	<p>
		...
		<input type="hidden" name="module[view_id]" value="<?php echo $item->view_id;?>" id="hidden_view_id">
		<input type="hidden" name="module[pos_name]" value="<?php echo $item->pos_name;?>" id="hidden_pos_name">
	</p>
</div>		
...
<script>	
	...
	$(function(){
		...
		$('#save').click(function(){
			...
			$.post('view_module.post.php',$('#view_item_container input,#view_item_container textarea,#view_item_container select').serializeArray(),function(data){				
				if(data != 'ok'){
					alert('保存失败!');			
				}
		...
		$('#a_quick_add').click(function(e){
			...
			$('#a_quick_add_save').click(function(e){
				...
				$('#ajax_resutl').load('quick_add_category.post.php',{'category[category_type]':cate_type,'category[name]':$('#text_quick_add').val()},function(data){
